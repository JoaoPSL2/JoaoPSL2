# Olá! Eu sou o João Pedro 👋

Estudante de **Sistemas de Informação** na Faculdade Impacta Tecnologia, focado em desenvolvimento de software, automação de processos, integração de APIs e criação de soluções eficientes.

---

### 🚀 Sobre Mim
- 🎓 **Graduação:** Cursando Bacharelado em Sistemas de Informação — Faculdade Impacta Tecnologia.
- 🎯 **Objetivo:** Buscando oportunidade de estágio na área de Tecnologia / Desenvolvimento de Software.
- 💡 **Interesses:** Desenvolvimento de Software, Ferramentas No-Code/Low-Code, Automações (n8n), APIs, Bancos de Dados e UX/UI.
- 💼 **Experiência Profissional:** Atuação prévia como Assistente Administrativo (Aprendiz) no Atacadão, com experiência em análise de dados financeiros, organização de relatórios e utilização avançada do Microsoft Excel.

---

### 🛠️ Tecnologias & Ferramentas em Estudo

#### No-Code / Low-Code & Automação:
- **Ferramentas:** n8n, FlutterFlow, Xano
- **Design & Prototipagem:** Figma

#### Desenvolvimento & Fundamentos:
- **Linguagens & Web:** HTML5, CSS3, Lógica de Programação
- **Integração & Dados:** APIs, Bancos de Dados
- **Controle de Versão:** Git, GitHub

---

### 📚 Formação Acadêmica

1. **Faculdade Impacta Tecnologia**
   - Bacharelado em Sistemas de Informação *(Em andamento)*
   - Foco: Desenvolvimento de sistemas, bancos de dados, APIs, No-Code/Low-Code, UX/UI, automação, IA, programação e engenharia de software.

2. **Universidade Paulista (UNIP)**
   - Bacharelado em Sistemas de Informação *(Interrompido para transferência para a Impacta)*

---

### 💼 Experiência

**Atacadão** — *Assistente Administrativo (Aprendiz)*
- mai de 2023 – out de 2024
- Atuação em rotinas administrativas, organização, atualização e análise de relatórios de vendas e dados financeiros.
- Gerenciamento de documentos, conferência e lançamento de notas fiscais.
- Acompanhamento de informações com Microsoft Excel.

---

### 📬 Conecte-se comigo
- **LinkedIn:** [João Pedro Silva Lopes](https://www.linkedin.com/in/jo%C3%A3o-pedro-silva-lopes-84a901349/)
- **Localização:** São Paulo, SP
